...
class ADS1256:
    def __init__(self):
        self.rst_pin = config_ADC1.RST_PIN
        self.cs_pin = config_ADC1.CS_PIN
        self.drdy_pin = config_ADC1.DRDY_PIN

    # Hardware reset
    def ADS1256_reset(self):
        config_ADC1.digital_write(self.rst_pin, GPIO.HIGH)
        config_ADC1.delay_ms(200)
        config_ADC1.digital_write(self.rst_pin, GPIO.LOW)
        config_ADC1.delay_ms(200)
        config_ADC1.digital_write(self.rst_pin, GPIO.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        config_ADC1.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC1.spi_writebyte([reg])
        config_ADC1.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    
    def ADS1256_WriteReg(self, reg, data):
        config_ADC1.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC1.spi_writebyte([CMD['CMD_WREG'] | reg, 0x00, data])
        config_ADC1.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        
    def ADS1256_Read_data(self, reg):
        config_ADC1.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC1.spi_writebyte([CMD['CMD_RREG'] | reg, 0x00])
        data = config_ADC1.spi_readbytes(1)
        config_ADC1.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,400000,1):
            if(config_ADC1.digital_read(self.drdy_pin) == 0):
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        ...
        config_ADC1.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC1.spi_writebyte([CMD['CMD_WREG'] | 0, 0x03])
        config_ADC1.spi_writebyte(buf)
        
        config_ADC1.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        config_ADC1.delay_ms(1) 
    ...
    def ADS1256_Read_ADC_Data(self):
        self.ADS1256_WaitDRDY()
        config_ADC1.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC1.spi_writebyte([CMD['CMD_RDATA']])
        # config_ADC1.delay_ms(10)

        buf = config_ADC1.spi_readbytes(3)
        config_ADC1.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_GetChannalValue(self, Channel, window, duration=1.0):
        ...
        intercept = 14386.52
        print ("Getting values in GetChannalValue")
        if ScanMode == 0:
            if Channel >= 8:
                return 0
            ...
            while time.time() - start_time < duration:
                ...
                counter += 1
                data_string += f"{window}, {Channel}, {timestamp}, {value}\n"
        ...
        print(f"Channel {Channel} with {counter} values for ADC1")
        return data_string
