...
class ADS1256:
    def __init__(self):
        self.rst_pin = config_ADC2.RST_PIN
        self.cs_pin = config_ADC2.CS_PIN
        self.drdy_pin = config_ADC2.DRDY_PIN

    # Hardware reset
    def ADS1256_reset(self):
        config_ADC2.digital_write(self.rst_pin, GPIO.HIGH)
        #config_ADC2.delay_ms(.2)
        config_ADC2.digital_write(self.rst_pin, GPIO.LOW)
        #config_ADC2.delay_ms(.2)
        config_ADC2.digital_write(self.rst_pin, GPIO.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        config_ADC2.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC2.spi_writebyte([reg])
        config_ADC2.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    
    def ADS1256_WriteReg(self, reg, data):
        config_ADC2.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC2.spi_writebyte([CMD['CMD_WREG'] | reg, 0x00, data])
        config_ADC2.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        
    def ADS1256_Read_data(self, reg):
        config_ADC2.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC2.spi_writebyte([CMD['CMD_RREG'] | reg, 0x00])
        data = config_ADC2.spi_readbytes(1)
        config_ADC2.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,400000,1):
            if(config_ADC2.digital_read(self.drdy_pin) == 0):
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        ...
        config_ADC2.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC2.spi_writebyte([CMD['CMD_WREG'] | 0, 0x03])
        config_ADC2.spi_writebyte(buf)
        
        config_ADC2.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
        config_ADC2.delay_ms(1) 
    ...
    def ADS1256_Read_ADC_Data(self):
        self.ADS1256_WaitDRDY()
        config_ADC2.digital_write(self.cs_pin, GPIO.LOW)#cs  0
        config_ADC2.spi_writebyte([CMD['CMD_RDATA']])
        # config_ADC2.delay_ms(10)

        buf = config_ADC2.spi_readbytes(3)
        config_ADC2.digital_write(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_GetChannalValue(self, Channel, window, duration=1.0):
        ...
        print(f"done with {Channel} with {counter} values for ADC2")
        return data_string
