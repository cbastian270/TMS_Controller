...
class ADS1256:
    def __init__(self):
        self.rst_pin = config.RST_PIN0
        self.cs_pin = config.CS_PIN0
        self.drdy_pin = config.DRDY_PIN0
 

    # Hardware reset
    def ADS1256_reset(self):
        config.digital_write0(self.rst_pin, GPIO.HIGH)
        #config.delay_ms0(200)
        config.digital_write0(self.rst_pin, GPIO.LOW)
        #config.delay_ms0(200)
        config.digital_write0(self.rst_pin, GPIO.HIGH)
    
    def ADS1256_WriteCmd(self, reg):
        config.digital_write0(self.cs_pin, GPIO.LOW)#cs  0
        config.spi_writebyte0([reg])
        config.digital_write0(self.cs_pin, GPIO.HIGH)#cs 1
    ...
    def ADS1256_WaitDRDY(self):
        for i in range(0,10000,1):
            if(config.digital_read0(self.drdy_pin) == 0):
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        ...
        config.digital_write0(self.cs_pin, GPIO.LOW)#cs  0
        config.spi_writebyte0([CMD['CMD_WREG'] | 0, 0x03])
    ...
    def ADS1256_GetChannalValue(self, Channel, duration = 1.0):
        data_list = []
        data_string = ""
